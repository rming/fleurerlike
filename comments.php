...
<input type="text" name="author" id="author" class="text" size="21"  placeholder="username"  />
</p>
<p>
<label for="mail"><?php _e('电子邮箱：&nbsp;'); ?><?php if ($this->options->commentsRequireMail): ?><span class="required">*</span><?php endif; ?></label>
<input type="text" name="mail" id="mail" class="text" size="21"  placeholder="username@mail.com"   />
</p>
<p>
<label for="url"<?php if ($this->options->commentsRequireURL): ?> class="required"<?php endif; ?>><?php _e('个人网站：&nbsp;'); ?></label>
<input type="url" name="url" id="url" class="text" size="21" placeholder="http://" value="<?php $this->remember('url'); ?>"<?php if ($this->options->commentsRequireURL): ?> required<?php endif; ?> />
</p>
<?php endif; ?>
<p>
...
<p>
<a class="button button-rounded button-tiny pull-right submit_button" onclick="document.getElementById('comment_form').submit();">提交评论</a>
</p>
</form>
</div>
<?php else: ?>
