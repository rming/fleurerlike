...
;(function($) {
	 ...
	 * 根据header创建目录内容
	 */
	var first_header_level = false;
	function create_toc(opts) {
		$(opts.documment_selector).find(':header').each(function() {
			if(first_header_level === false) {
				first_header_level = parseInt(this.nodeName.substring(1), 10);
			}

			var level = parseInt(this.nodeName.substring(1), 10);
			if(first_header_level > 1) {
				level = level - first_header_level + 1
			}
	...
	$.fn.ztree_toc.defaults = {
		...
		_headers: [],
		_header_offsets: [],
		_header_nodes: [{ id:1, pId:0, name:"[TOC]",open:true}],
		debug: true,
		/*
		 * 使用标题作为anchor
		 * create table with head for anchor for example: <h2 id="#Linux基础">Linux基础</h2>
			...
			callback: {
				beforeClick: function(treeId, treeNode) {
					...
					if($.fn.ztree_toc.defaults.is_highlight_selected_line == true) {
						$('#' + treeNode.id).fadeOut("quick" ,function() {
						    // Animation complete.
							$(this).show();
						});
					}
